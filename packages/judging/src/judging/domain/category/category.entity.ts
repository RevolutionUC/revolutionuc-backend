...
export class Category {
  ...
  assignSubmissionToGroup(submission: Submission, group: Group) {
    group.assignSubmission(submission);
  ...
  assignSubmissionsToGroups() {
    ...
    for (let i = 0; i < this.groupsPerSubmission; i++) {
      ...
      shuffledSubmissions.forEach((submission) => {
        ...
        this.assignSubmissionToGroup(submission, nextGroup);
      });
    }
  }

  assignJudgeToGroup(judge: Judge, group: Group) {
    group.assignJudge(judge);
    this.judges = this.judges.filter((j) => j.id !== judge.id);
  }
  ...
  assignJudgesToGroups() {
    ...
    shuffledJudges.forEach((judge) => {
      ...
      this.assignJudgeToGroup(judge, nextGroup);
    });
  }
